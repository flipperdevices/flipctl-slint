...
///
/// Decimal: 1 G is 10^9 bytes, which is what a desktop's system monitor says about
/// the same machine. procfs counts its kB as 1024 bytes, so the conversion goes
/// through bytes rather than dividing the kB figure by 1000.
fn size(kb: u64) -> String {
    let bytes = kb as f64 * 1024.0;
    if bytes >= 1e9 {
        format!("{:.1}G", bytes / 1e9)
    } else if bytes >= 1e6 {
        format!("{}M", (bytes / 1e6).round() as u64)
    } else {
        format!("{}K", (bytes / 1e3).round() as u64)
    }
}
...
/// its views. The page you are on is not offered again. The middle slot is the
/// Power key and carries nothing, here as everywhere; closing is the Back key's,
/// which flipctl answers itself.
...
fn main() -> Result<(), slint::PlatformError> {
    ...
    ui.on_keyed(move |text, down| {
        ...
        match key {
            ...
            Key::Escape => m.page = Page::Cpu,
            Key::View => m.page = Page::Memory,
