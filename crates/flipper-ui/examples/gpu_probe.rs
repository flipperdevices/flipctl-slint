...
//!
//! The hosting path depends on three things being present: importing a dmabuf as an
//! EGL image, a context with no surface to render from, and single-channel render
//! targets, which are what make a frame 36KB to read back instead of 147KB. This
//! reports all three, the formats GBM will allocate, and which driver answered.
...
fn main() {
    ...
    // The formats GBM will allocate for the compositor to copy into. An import that
    // is refused here is refused the same way in flipctl, with the same message, so
    // this is the question worth asking of a machine before hosting anything on it.
        ...
        Ok(mut converter) => {
            for name in [b"XR24", b"AR24", b"AB24", b"XB24"] {
                ...
                match converter.allocate(
                    ...
                    Ok(target) => println!(
                        "import {label}: {}x{}, modifier {:#018x}, {} plane(s)",
                        target.w,
                        target.h,
                        target.modifier,
                        target.planes.len()
                    ),
                    Err(e) => println!("import {label}: refused ({e})"),
                }
            }
        }
