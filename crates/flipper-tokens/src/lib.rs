//! The generated themes, from tokens.toml.
//!
//! A build-time library rather than one crate's build script, so flipctl and a
//! hosted app generate the same theme from one parse of one file. Nothing outside
//! the generated modules may hold a colour or a layout constant;
...
/// tokens.toml, parsed.
pub fn parse(src: &str) -> toml::Value {
    src.parse().expect("tokens.toml is not valid TOML")
}
...
/// as long as it did.
pub fn write_if_changed(path: &std::path::Path, contents: &str) {
...
pub fn rust_theme(doc: &toml::Value) -> String {
...
pub fn slint_theme(doc: &toml::Value) -> String {
